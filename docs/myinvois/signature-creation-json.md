# Signing a JSON document (LHDN reference)

> **Source:** scraped from the LHDN MyInvois developer portal page
> "Signing a JSON document" (XAdES-BES enveloped, JSON variant).
> This is the **authoritative reference** for `backend/src/lib/signing.ts`
> and the `verify-signing` / `verify-signature` tests. Code samples are
